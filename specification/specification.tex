...
After antidote has restarted, due to a scheduled restart or a crash, it needs to reconstruct a stable view of the database. We assume that the Journals in each shard might have incomplete information about transactions. 

The goal of the recovery is to reconstruct information that is necessary for the database to run correctly and ensure that the different invariants are true.
...
- High Watermark
- Low Watermark
- Transactions that have a prepare record in each participating shard have a commit record written in each relevant Journal.
- Transactions that are missing a prepare record have an abort message written in each relevant Journal.

Recovery is initiated the Transaction Manager at each shard. 
The recovery reads the Journal entirely. 
