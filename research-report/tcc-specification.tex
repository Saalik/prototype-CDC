...
be unsafe.
To avoid this, a transaction should wait if a shard is missing updates with
respect to another.

The Cure protocol \cite{rep:pro:sh182} is what ensures the TCC properties.
Cure has two main objectives:
\begin{enumerate}%[(1)]
\item Ensure that transactions in a DC commit atomically, and in a
  total order across all shards of that DC\@.
  It uses the Clock-SI design for this purpose
  \cite{rep:pan:1723}.
\item Ensure that updates are observed in a causally-consistent
  order within a DC\@.
...
updates.
This makes the representation simpler and more compact and enables the 
...
\textit{Min-Dependency} represents the oldest snapshot any
...
\textit{Max-Committed} is the last commit time recorded in the Journal.
...
Hence, our first invariant is $\mathit{Low-Watermark} \le \mathit{Checkpoint Time}$.
...
no data is lost.
First, if a Checkpoint exists there should be a \textit{Checkpoint} record present in
the Journal.
...
satisfies the dependency, \textit{i.e.}, such that \emph{c $\le$ dep} and there
